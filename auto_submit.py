...
def run_dreamina(cmd_args, record_key=None, dry_run=False):
    ...
    if submit_id and record_key:
        ...
        with open(f, "a", encoding="utf-8") as fp:
            fp.write(f"{datetime.now().isoformat()} | {submit_id}\n")
    return submit_id
...
def cmd_videos(draft=False, vip=False, ep_filter=None, dry_run=False):
    ...
    for eid, ep_data in eps.items():
        ...
        for seg in segs:
            ...
            run_dreamina(cmd_args, record_key="视频生成", dry_run=dry_run)
            time.sleep(2)
...
def cmd_poll():
    for rf in DIRS["任务记录"].glob("*_submit_ids.txt"):
        ...
        for line in rf.read_text(encoding="utf-8").splitlines():
            ...
            subprocess.run(["dreamina", "query_result",
                f"--submit_id={sid}", f"--download_dir={DIRS['输出']}"])
            time.sleep(0.5)
