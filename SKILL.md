...
剧本Agent（Screenwriter）
    ...
    【章节边界检查】
    - 本集反派/冲突级别是否匹配 series_bible.md 中该章定义的对手范围？
    - 超出范围 → 阻断并提示：要么调整本集设计，要么修改 series_bible.md 章节结构
    【剧本输出】
    输出：每集完整剧本（场景描述·角色行为·情绪节拍·剧情逻辑）
...
【A. 章节一致性检查（剧本定稿后）】
检测规则（阻断级）：
❌ 反派越级：本集反派级别超出 series_bible.md 中该章定义的对手范围
   （例：第一章定义"同辈师兄弟"，但剧本出现"长老级"反派 → 阻断）
❌ 弧线偏离：本集核心冲突与该章核心命题无关
   （例：第一章命题"我还活着"，但剧本在讲域外势力入侵 → 阻断）
❌ 前集断裂：本集开场与上集结尾的剧情状态不衔接
   （例：上集结尾角色A重伤倒地，本集开场角色A毫发无损 → 阻断）

检测规则（警告级，不阻断）：
⚠️ 伏笔遗忘：上集埋下的伏笔在本集既未推进也未呼应
